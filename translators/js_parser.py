...
class JSToIR:
        ...
        if node_type == 'FunctionDeclaration':
            ...
            if node.get('declarations'):
                ...
                return IRBuilder.assign(
                    ...
                    self._convert_node(decl['init']) if decl.get('init') else IRBuilder.literal(None)
                )
    ...
    def _convert_function(self, node: Dict[str, Any]) -> IRNode:
        ...
        body = [self._convert_node(stmt) for stmt in node.get('body', {}).get('body', []) if stmt]
    ...
    def _convert_if(self, node: Dict[str, Any]) -> IRNode:
        ...
        body = [self._convert_node(stmt) for stmt in node.get('consequent', {}).get('body', []) if stmt]
        orelse = []
        if node.get('alternate'):
            if node['alternate']['type'] == 'BlockStatement':
                orelse = [self._convert_node(stmt) for stmt in node['alternate'].get('body', []) if stmt]
            else:
    ...
    def _convert_for(self, node: Dict[str, Any]) -> IRNode:
        ...
        target = self._convert_node(node['init']['declarations'][0]['id']) if node.get('init') else IRBuilder.name('i')
        iter = self._convert_node(node['test']) if node.get('test') else IRBuilder.literal(10)  # placeholder
        body = [self._convert_node(stmt) for stmt in node.get('body', {}).get('body', []) if stmt]
